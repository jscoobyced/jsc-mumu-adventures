...
- The file `frontend/src/levels/index.ts` is used to configure the relationships between levels
...
To convert a .tmj file, navigate to the `frontend` folder and run the command:

```
yarn gen-map A/1/map.tmj
```

This will create (or replace) the files in `frontend/src/levels/A/1/`. It will create a TypeScript file for each tileset in the map.
You might need to adjust the `frontend/src/levels/A/1/index.ts` to add or remove some tilesets. Refer to the [Create a Legend of Zelda Style Game with Javascript](https://www.youtube.com/watch?v=zogxGGDJ2Ok) course to understand the different layers.
