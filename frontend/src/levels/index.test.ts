...
describe("levels/index", () => {
  describe("levelConfig", () => {
    ...
    it("should have correct level count", () => {
      expect(levelConfig.length).toBe(3);
    });
    ...
    it("should have levelA1 connected to levelA2 on the right", () => {
      ...
      expect(levelA1Config?.connectedLevels?.[0].direction).toBe(LevelDirection.RIGHT);
    ...
    it("should have levelA2 connected to levelA1 on the left", () => {
      ...
      expect(levelA2Config?.connectedLevels?.length).toBe(2);
      expect(levelA2Config?.connectedLevels?.[0].direction).toBe(LevelDirection.LEFT);
    ...
    it("should use proper level direction enum values", () => {
      levelConfig.forEach((config) => {
        config.connectedLevels?.forEach((connection) => {
          expect(Object.values(LevelDirection).includes(connection.direction)).toBe(true);
        });
      });
    });
